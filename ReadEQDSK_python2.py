...
from __builtin__ import *  # <- python 2.
...
def ReadEQDSK(in_filename):
	...
	#first line
	rboxlength = float(next(token))
	zboxlength = float(next(token))
	R0EXP = float(next(token))
	rboxleft = float(next(token))
	zmid   = float(next(token)) #(maxygrid+minygrid)/2
	
	#second line
	Raxis = float(next(token))
	Zaxis = float(next(token))
	psiaxis = float(next(token)) # psi_axis-psi_edge
	psiedge = float(next(token)) # psi_edge-psi_edge (=0)
	B0EXP = float(next(token)) # normalizing magnetic field in chease
	...
	nLCFS = int(next(token))
	nlimits = int(next(token))
	...
	if nLCFS > 0:
		...
		for ii in range(nLCFS):
			R[ii] = float(next(token))
			Z[ii] = float(next(token))
	else:
		R = [0]
		Z = [0]
	...
	if nlimits > 0:
		...
		for ii in range(nlimits):
			R_limits[ii] = float(next(token))
			Z_limits[ii] = float(next(token))
	else:
		R_limits = [0]
		Z_limits = [0]
