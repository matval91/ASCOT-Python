...
class eqdsk():
	def __init__(self,comment,switch,nrbox,nzbox,rboxlength,zboxlength,zmid,R0EXP,rboxleft,Raxis,Zaxis,psiaxis,psiedge,B0EXP,Ip,T,p,TTprime,pprime,psi,q,nLCFS,nlimits,R,Z,R_limits,Z_limits,R_grid,Z_grid,psi_grid,rhopsi):
...
def ReadEQDSK(in_filename):
		...
		for jj in ii.split():
                        if re.finditer(d, jj):
                                for ind_s,s in enumerate(re.finditer(d,jj)):
                                        el1=jj[0:s.span()[0]+1-ll*ind_s]
                                        if el1[-4]!='E':
                                                el1=jj[0:s.span()[0]+2-ll*ind_s]
                                        data.append(el1)
                                        jj=jj[len(el1):]
                                        ll=len(el1)
                                        el1=''
	...
	#comment = firstline[0]+' '+firstline[1]+' '+firstline[2]+' '+firstline[3]
	switch = firstline[-3]
	nrbox = int(firstline[-2])
	nzbox = int(firstline[-1])
	...
	out = eqdsk(comment,switch,nrbox,nzbox,rboxlength,zboxlength,zmid,R0EXP,rboxleft,Raxis,Zaxis,psiaxis,psiedge,B0EXP,Ip,T,p,TTprime,pprime,psi,q,nLCFS,nlimits,R,Z,R_limits,Z_limits,R_grid,Z_grid,psi_grid,rhopsi)
	return out
